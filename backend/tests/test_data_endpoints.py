"""The data endpoints: open, wage-free, reading Boord read-only."""
import pytest
...
def test_answers_without_credentials(client, path):
    # The app has no sign-in: reaching the port is the whole of the access
    # control (the server binds 127.0.0.1 and `tailscale serve` publishes
    # it). A 401/403 here would mean auth had crept back in.
    assert client.get(path).status_code == 200


def test_dashboard_summary_is_wage_free(client):
    r = client.get(f"/api/dashboard/summary?{_RANGE}")
...
def test_dashboard_supplier_filter(client):
    ...
    unfiltered = client.get(f"/api/dashboard/summary?{_RANGE}").json()
    assert {w["worker_id"] for w in unfiltered["workers"]} == {"001", "002"}


def test_dashboard_sorting_and_derived_figures(client):
    body = client.get(f"/api/dashboard/summary?{_RANGE}").json()
...
def test_dashboard_period_is_required(client):
    assert client.get("/api/dashboard/summary").status_code == 422


def test_dashboard_empty_period_is_not_an_error(client):
    body = client.get("/api/dashboard/summary?period_start=2019-01-01&period_end=2019-12-31").json()
    assert body["workers"] == [] and body["blocks"] == []
    assert body["active_teams"] == 0


def test_lots_lists(client):
    assert len(client.get("/api/lots/in-transit").json()) == 1
    received = client.get("/api/lots/received").json()
    assert len(received) == 1 and received[0]["slip_number"] == "261001-003"


def test_system_settings_and_suppliers(client):
    ...
    names = {x["name"] for x in client.get("/api/suppliers").json()}
...
def test_analysis_weather_risk_render(client):
    a = client.get("/api/analysis/summary")
    assert a.status_code == 200 and a.json()["current_year"] == 2026
    w = client.get("/api/weather/history")
...
def test_analysis_reports_the_anchor_it_was_built_against(client):
    ...
    would show."""
    assert _analysis(client)["season_anchor"] == {"month": 8, "day": 1}


def test_season_is_the_anchored_year_not_the_calendar_year(client):
...
def test_analysis_excludes_another_growers_blocks(client):
    ...
    dash = client.get(f"/api/dashboard/summary?{_RANGE}").json()
...
# --------------------------------------------------------------------------- #
def test_historical_harvest_data_workbook(client):
    ...
    r = client.get("/api/reports/historical-harvest-data")
    assert r.status_code == 200, r.text
